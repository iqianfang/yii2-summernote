...
class Summernote extends InputWidget
{
    ...
    /** @var bool Set to true to upload images to server */
    public $uploadToLocalServer = true;
    ...
    public function run()
    {
        ...
        if ($this->uploadToLocalServer) {
            $this->clientOptions['callbacks']['onImageUpload'] = "function(files) {
                ...
                summernoteLocalServerUploader.signEndpoint = '".Url::to(['attachment/img-upload'])."';
                summernoteLocalServerUploader.files = files;
                summernoteLocalServerUploader.editor = editor;
                if(files.length > 10){
                    alert('单次图片上传不能超过10个！');
                    return;
                }
                summernoteLocalServerUploader.sendImage(0);
    ...
    private function registerAssets()
    {
        ...
        if ($this->uploadToLocalServer) {
            SummernoteLocalServerAsset::register($view);
        }
